...
function FunctionLoginTXWY:RequestAuthAccToken()
    ...
    if(sid)then
        ...
        local url = string.format("%s/auth?sid=%s&p=%s&sver=%s&cver=%s&lang=%s",NetConfig.NewAccessTokenAuthHost[1],sid,plat,version,clientCode,
            ApplicationInfo.GetSystemLanguage())
